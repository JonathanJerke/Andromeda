...
INT_TYPE tSelect(struct sinc_label  f1, INT_TYPE Ve, INT_TYPE type, enum division usr, enum division usa, INT_TYPE testFlag){
    ...
    if ( type ) {
        ...
        for ( sp = 0; sp < spins(f1, usr+Ve);sp++){
            f1.tulip[totalVector].Current[rank] = 0;
            tBuildIrr(rank, f1, type, usa, sp, totalVector, rank);
            tCycleDecompostionListOneMP(rank, f1, totalVector, rank, NULL,usr+Ve, sp, f1.rt->vCANON, part(f1,usr+Ve), -1);
        }
...
INT_TYPE tGreaterDivide ( INT_TYPE translateFlag ,double sumPart, double realPart,struct sinc_label  f1,enum division spiralBra,enum division Op , enum division spiralKet){
    ...
    if ( translateFlag  == 1){
        printf (" %1.6f | PSI > + %1.6f H | PSI > \n",sumPart,realPart);
    }else if ( translateFlag == 0 ){
        printf (" %1.6f H | PSI >\n", realPart);
    }else if ( translateFlag == -1 ){
        printf (" %1.6f H | PSI > - <H> | PSI >\n", realPart);
    }
    //////bra///
    //consumer of totalVector..
    pHXpX(-1, f1,  spiralBra, Op, translateFlag ,sumPart, realPart, 0.0, spiralKet, f1.rt->vCANON , f1.rt->ALPHA,f1.tulip[spiralBra].Partition);
    return 0;
};


INT_TYPE tLesserDivide ( INT_TYPE translateFlag ,double sumPart, double realPart,struct sinc_label  f1,enum division spiralBra,enum division OpSpiral , enum division spiralKet){
    INT_TYPE *iii[2][2];
...
#pragma omp parallel for private (itok,rank)
        ...
        for (itok = 0;  itok < nitok*nik ; itok++){
            ...
            rank = 0;
                ...
                printf("ham%d ket%d rank%d\n", itok/nik+1,itok%nik+1,rank);
                fflush(stdout);
                //consumer of totalVector..
...
//                            printf("\n\n");
                            ...
                            tCycleDecompostionListOneMP(-2,f1, eigenList,0, pointers[rank],el+iii   , cmpl, f1.rt->vCANON, part(f1, el+iii), f1.rt->powDecompose);
