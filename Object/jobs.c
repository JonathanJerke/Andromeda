...
//            else
            ...
            tCycleDecompostionChromaticOneMP( f1.f, totalVector, 0, NULL,eigenVectors , cmpl, c1->rt.vCANON, c1->rt.ALPHA, part(f1.f,eigenVectors), c1->rt.powDecompose);
        }
...
double singlekrylov ( struct calculation *c1, struct field f1){
    INT_TYPE EV = 0;
#ifndef APPLE
    f1.i.qFloor = 0;//countLinesFromFile(c1,f1,0,&f1.i.iRank, &f1.i.xRank);
    f1.i.iRank = 1;
    //count canonical-rank...
    f1.i.nStates =1 ;
    f1.i.Iterations = 1;
    ...
    iModel(c1,&f1);
//    for ( fi =0 ; fi < f1.i.files ; fi++){//thinking... one vector...
//        tLoadEigenWeights (c1,f1, f1.i.fileList[fi], &EV,f1.f.user,0);
//    }
    tLoadEigenWeights (c1,f1, f1.i.file2, &EV,eigenVectors,0);
    ...
    enum division target= nullName,OpSpiral = defSpiralMatrix(&f1.f, Iterator);//all allocaitons are forgotten after procedure ends!
    target = f1.f.tulip[OpSpiral].name;
    {
        double norm = magnitude(f1.f, eigenVectors );
        if ( norm > c1->rt.TARGET ){
            printf("Normed from %f\n", norm );
    ...
    tGreaterDivide(c1->i.shiftFlag, c1->i.shiftVector[0][0], c1->i.shiftVector[0][1], f1.f,eigenVectors,target,eigenVectors);
    pMatrixElements( f1.f, eigenVectors, nullName, eigenVectors, NULL, &vhhv);
    f1.f.tulip[eigenVectors].value.value = vhhv;
    print(c1,f1,1,0,1,eigenVectors);
    fModel(&f1.f);
    return 0;
}
...
int run (INT_TYPE argc , char * argv[]){
   ...
   // test2();
    ...
    else if ( c.rt.phaseType == singleKrylov ){//c
        singlekrylov(&c,f);
    }
