...
double vale ( struct sortClass * f ){
    ...
    double value =  f->str[0][mm[0] + mm[3]*f->n1[0]];
    if ( SPACE > 1 )
        value += f->str[1][mm[1] + mm[4]*f->n1[1]] ;
    if ( SPACE > 2 )
        value += f->str[2][mm[2] + mm[5]*f->n1[2]];
    return value;
}
...
INT_TYPE t1BodyConstruction(struct calculation * c1, enum division eigen){
    ...
    INT_TYPE space,spacer,N2 = N1*N1,i,ii,iii,iv,v;
    double ar[N2],w[N1];
...
#ifdef OMP
#pragma omp parallel for private (v,rank,i,ii,spacer) schedule(dynamic,1)
#endif
            ...
            for ( v = 0 ; v < N2 ; v++){
                ...
                // if ( l2*l2 > i*i+ii*ii )
                {
                    ...
                    for ( spacer = 0 ; spacer < SPACE ; spacer++)
                        cblas_dcopy(N1, ar+i*N1, 1, streams(f1,diagonal1VectorA,rank,spacer), 1);

                    f1->sinc.tulip[diagonal1VectorA].Current[rank] = 1;
                    for ( spacer = 0 ; spacer < SPACE ; spacer++)
                        cblas_dcopy(N1, ar+ii*N1, 1, streams(f1,diagonal1VectorB,rank,spacer), 1);
...
#ifdef OMP
#pragma omp parallel for private (v,rank,i,ii,iii,spacer) schedule(dynamic,1)
#endif
            ...
            for ( v = 0 ; v < N2*N1 ; v++){
                ...
                for ( spacer = 0 ; spacer < SPACE ; spacer++)
                    cblas_dcopy(N1, ar+iii*N1, 1, streams(f1,diagonal1VectorC,rank,spacer), 1);
...
#ifdef OMP
#pragma omp parallel for private (v,rank,i,ii,iii,iv,spacer) schedule(dynamic,1)
#endif
            ...
            for ( v = 0 ; v < N2*N2 ; v++){
                ...
                for ( spacer = 0 ; spacer < SPACE ; spacer++)
                    cblas_dcopy(N1, ar+iv*N1, 1, streams(f1,diagonal1VectorD,rank,spacer), 1);
...

    {
        ...
        for ( cmpl =  0; cmpl < 1 ; cmpl++ ) {
            ...
            if ( cmpl == 0 ){
                ...
                tCycleDecompostionOneMP(0, f1, build, 0, eigen, 0, f1->mem1->rt->CANON, 1, -1);
                
                if ( bootBodies > one ){
                    
                    
                    if ( c1->rt.bodyType > electron ){
                        if ( CanonicalRank(f1, interactionExchangePlus, 0)){
                            tEqua(f1, squareTwo,0, ocean(0,f1, interactionExchangePlus,0,0),0);
                            tCycleDecompostionOneMP(0, f1, interactionExchangePlus, 0,squareTwo, 0, f1->mem1->rt->CANON, 1 , -1);
                    ...
                    if ( CanonicalRank(f1, interactionExchange, 0)){
                        ...
                        tCycleDecompostionOneMP(0, f1, interactionExchange, 0,squareTwo, 0, f1->mem1->rt->CANON, 1 , -1);
                        tSumMatrices(f1, build, squareTwo);//B2:1 || B3 : 3
                        tCycleDecompostionOneMP(0, f1, build, 0, eigen, 0, f1->mem1->rt->CANON, 1, -1);
                    }
                }
                ...
                if ( CanonicalRank(f1, kinetic,1 ) ){
                    ...
                    tSumMatrices(f1, build, copy);// B x (1+S) * 3
                    tCycleDecompostionOneMP(0, f1, build, 0, eigen, 1, f1->mem1->rt->CANON, 1, -1);
                }
            }
        }
    }
...
INT_TYPE tFoundationLevel( struct field * f1, enum division A , double lvlm, double lvlx,INT_TYPE ops,enum division build,INT_TYPE xB, double lvl1, double lvl2, double lvl3,INT_TYPE *mmm, INT_TYPE irrep,double seekPower){
    ...
    ///// GRID
    ////////GRID
    INT_TYPE i,j,k,r1,r2,r3,space,ii,jj,kk,xx[3];
                                    ...
                                    for ( r3 = 0; r3 < nG ; r3++)
                                        for ( k = 0 ; k < xx[2] ; k++)
                                            ...
                                            if ( streams(f1,foundationStructure,1,2)[k+r3*n1[2]] > 0.   )
                                            {
                                                ...
                                                value += streams(f1,foundationStructure,0,1)[j+jj*n1[1]];
                                                
                                                value +=  streams(f1,foundationStructure,0,2)[k+kk*n1[2]];
                                                if ( vaMax < value )
                                                    vaMax = value;
                                                ...
                                                if ( lvlm <= value ){
                                                    {
                                                        ...
                                                        if ( tIR ( bd,ii,jj,kk,irrep)){
                                                            ...
                                                            if ( ops  == 0 ){
#if VERBOSE
                                                                printf("%d-->%d : %d %d %d :: %d %d %d :: %f\n",irrep,classicalBasisSize,i,j,k,ii+1,jj+1,kk+1,value);
#endif
...
INT_TYPE tFilter(struct field * f1, INT_TYPE Ve, INT_TYPE irrep, enum division usr){
    INT_TYPE ii,cmpl=0,rank,nP = tPerms(f1->body);
    assignCores(f1, 2);
#ifdef OMP
...
//#else
        if ( irrep && bodies(f1, usr+ii ) > one  ){
            for ( cmpl = 0 ; cmpl < spins(f1, usr+ii) ; cmpl++){
                ...
                tCycleDecompostionOneMP(rank, f1,permutationVector, rank, usr+ii,cmpl,f1->mem1->rt->vCANON, part(f1,usr+ii), 1);
            }
        }
...
INT_TYPE tSelect(struct field * f1, INT_TYPE Ve, INT_TYPE type, enum division usr, enum division usa, INT_TYPE testFlag){
    ...
    if ( type ) {
        ...
        for ( sp = 0; sp < spins(f1, usr+Ve);sp++){
            ...
            tCycleDecompostionOneMP(rank, f1, permutationVector, rank, usr+Ve, sp, f1->mem1->rt->vCANON, part(f1,usr+Ve), -1);
        }
...
INT_TYPE tCollect (struct field * f1, INT_TYPE irrep,enum division usz, INT_TYPE target,double seekPower){
    ...
    INT_TYPE space, nG = tSize(f1->body);
        ...
        while(1){
            ...
            if ( ct >= va*target && ct <= target*va*vx)
            {
                ...
                for ( i = 0; i < ct ; i++){
                    ...
                    for ( space = 0; space < SPACE ; space++)
                        cblas_dcopy(n1[space], streams(f1, build, 0 , space)+(mm[space])*n1[space]+(mm[space+3])*n1[space]*n1[space],1,streams(f1,diagonalVectorA,0,space),1);
...
#if 1
                printf("%d :: \t %f\t %d i%d\n",iii+1,sqrt(temp2/temp), CanonicalRank(f1, usz+(expon)*foundation+iii, 0),CanonicalRank(f1, usz+(expon)*foundation+iii, 1));
#endif
                f1->sinc.tulip[usz+(expon)*foundation+iii].value3 = sqrt(temp2);
                fflush(stdout);
            }
...
INT_TYPE tSAboot(struct calculation *c1){
    struct field *f1 = &c1->i.c;
    {
        ...
        INT_TYPE i,j,k,nG = tSize(f1->body), v,r2,info,r,rank=0,space,irrepm,LN2[3];
        INT_TYPE hits[nG];
        for ( i = 0 ; i < nG ; i++)
            hits[i] = 0;
        length(f1, eigen, LN2);
        ...
        for ( irrepm = 0 ; irrepm <  nG ; irrepm++){
                ...
                for ( space = 0 ; space < SPACE ; space++)
                    for ( r = 0 ; r < n1[space]; r++)
                        {
                    streams(f1,foundationStructure,0,space)[r+(irrepm)*n1[space]] = 0;
                        ...
                        for ( p = 0 ; p < 6 ; p++){
                            ...
                            for ( irrepm = 0; irrepm < nG ; irrepm++){
                                for ( r2 = 0; r2 < CanonicalRank(f1, permutationVector, rank); r2++)
                                    for ( space = 0; space < SPACE ; space++){
                                        cblas_daxpy(n1[space], get1(f1->body, irrepm+1, r2), streams(f1, permutationVector,rank,space)+r2*n1[space], 1, streams(f1,diagonalVectorA, rank, space),1);
                           ...
                           //     printf( "%d %d %f\n", irrepm+1,CanonicalRank(f1, diagonalVectorA,rank),vo);
                                ...
                                if ( vo > 0.01 && hits[irrepm] < xyz){
                                    ...
                                    tHXpX(rank, f1, Ha, 0, 1.0, 0, diagonalVector, 1e-6, 1);
           ...
           // for ( m = 0; m < quantumBasisSize ; m++)
            ...
            for ( s1 = 0 ; s1 < 1; s1++){
                ...
                do {
                    ...
                    for ( cmpl = 0; cmpl < spins(f1,name(f1,leftP))  ;cmpl++)
